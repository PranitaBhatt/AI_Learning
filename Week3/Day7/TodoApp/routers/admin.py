...
router = APIRouter(
    prefix='/admin', #will simply divides enpoints of auth user to that of default in SwaggerUI
    tags=['admin']
)
...
async def delete_todo(user:user_dependency,db:db_depedency,todo_id:int=Path(gt=0)):
    if user is None or user.get('user_role')!='admin':
        raise HTTPException(status_code=401,detail='Authentication Failed')
